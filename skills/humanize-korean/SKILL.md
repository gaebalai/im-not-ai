...
1. 사용자 프로젝트의 작업 디렉토리(현재 `cwd`) 하위 `_workspace/` 확인. plugin 모드면 사용자 프로젝트, 로컬 모드면 이 리포 루트.
2. 분기:
...
taxonomy_path: references/ai-tell-taxonomy.md   # 본 스킬 디렉토리 기준. plugin/.claude 양쪽 호환. LLM이 못 찾으면 Glob "**/ai-tell-taxonomy.md" 로 탐색.
...
playbook_path: references/rewriting-playbook.md   # 본 스킬 디렉토리 기준. 못 찾으면 Glob "**/rewriting-playbook.md".
```
...
**모든 Agent 호출은 `model: "opus"` 명시.** 파일 기반 데이터 전달, `_workspace/{run_id}/` 하위에 번호 접두사 파일 저장.

**에이전트 정의 위치 (둘 다 동일 — plugin/local 양쪽 호환):**
- 정본: 리포 루트의 `agents/*.md`
- 미러: `.claude/agents/*.md` (루트로 향하는 심볼릭 링크)
- 정의된 에이전트 6명: korean-ai-tell-taxonomist, ai-tell-detector, korean-style-rewriter, content-fidelity-auditor, naturalness-reviewer, humanize-web-architect
