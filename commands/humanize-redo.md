...
가장 최근 `_workspace/{run_id}/`를 찾아 `humanize-korean` 스킬 Phase 3(윤문) 또는 Phase 4(검증)부터 재호출한다.

## 사용자 지시
$ARGUMENTS

## 동작
1. `_workspace/`에서 가장 최신 `run_id` 디렉토리 식별 (없으면 "이전 실행이 없습니다. `/humanize`로 시작하세요" 안내).
2. 사용자 지시 파싱:
   ...
   - **롤백 요청** ("이 변경 되돌려줘") → 해당 edit을 fidelity-auditor 롤백 명령으로 처리
