...
이 하네스는 그 한글 고유 패턴을 SSOT로 정리하고, 탐지·윤문·내용 감사·자연스러움 검증을 분리된 에이전트로 수행한다.
...
이 리포지토리는 [Claude Code](https://claude.com/claude-code) 하네스로 설계됐다. 프로젝트 루트를 Claude Code 작업 디렉토리로 지정하면 오케스트레이터 스킬이 자동 트리거된다.
...
- 본 도구는 "AI 탐지기 우회(Undetectable AI)"가 아니라 **한글 글쓰기 품질 개선**을 목적으로 한다.
